...
<?php
    ...
    if(loggedin()) {
		...
		if($u_admin>=1) {
			echo "<div class='contentbox objava mt'>
				<a href='/adminpanel/'><div class='title'> 
					<span>Admin Panel</span>
				</div></a><div class='zbuttons mt-10'>
					<a href='/adminpanel/'>Podešavanja</a>
					<a href='/adminpanel/clanovi/'>Članovi</a>
					<a href='/adminpanel/poruke/'>Poruke</a>
					<a href='/adminpanel/prodavnica/'>Prodavnica</a>
					<a href='/adminpanel/statistika/'>Statistika</a>
				</div>
			</div>";
		}
    }
...
?>
	<div class="contentbox">
        <form action="/pretraga/" method="post">
			<input type="text" placeholder="Pretraga..." name="search" class="inputtext" style="width: calc(100% - 40px);"><input type="submit" name="submit" class="btnsrc" value="" id='src-btn'>
		</form>
	</div>

    <div class="contentbox objava mt">
        <a href='/dogadjaji/'><div class="title"> 
            <span>Kalendar događaja</span>
        </div></a><div id="kalendar">
		<?php
			$date = time();
			$month = date('m',$date);
			$year = date('Y',$date);
